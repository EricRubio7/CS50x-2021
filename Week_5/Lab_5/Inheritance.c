...
person *create_family(int generations)
{
    ...
    if (generations > 1)
    {
        ...
        //Set the newPerson parent (which is a pointer), to a newly created person struct
        newPerson->parents[0] = create_family(generations - 1);
        newPerson->parents[1] = create_family(generations - 1);

        // TODO: Randomly assign child alleles based on parents
        //Set newPerson allele, by following it's parent pointer in to the parent-person struct,
        //and accessing it's allele property
